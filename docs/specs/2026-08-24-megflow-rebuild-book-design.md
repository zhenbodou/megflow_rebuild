> 当前要求更新（2026-09-06）：用户要求完整 Rust 功能与业务逻辑对齐，且宏教学从入门到能独立维护，包含涉及的 crate。
> 本文保留为早期设计记录；其中“核心子集”“原版部分模块超范围”等范围限制不再代表当前完成标准。
> 当前差距与验证证据以 `book/src/part0/ch04-completeness-audit.md` 和 Part 2 的原版宏清单为准，完整功能仍在补齐。
