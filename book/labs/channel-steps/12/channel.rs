...
pub use conversion::{add_cvt_func_impl, CvtF};
mod typed;
...
impl Sender {
    ...
    pub async fn send_any(&self, message: SealedEnvelope) -> Result<()> {
        ...
        let message = convert(self.conversion, message).await;
        match &self.inner {
...
impl Receiver {
    ...
    pub async fn recv_any(&self) -> Result<SealedEnvelope> {
        ...
        Ok(convert(self.conversion, message).await)
    }
...
async fn convert(function: Option<CvtF>, message: SealedEnvelope) -> SealedEnvelope {
    if message.is::<flow_message::DummyEnvelope>() {
        return message;
    }
    match function {
        None => message,
        Some(function) => tokio::task::spawn_blocking(move || function(message))
            .await
            .unwrap(),
    }
}
