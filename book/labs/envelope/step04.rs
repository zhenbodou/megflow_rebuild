/*!
 * flow-message · envelope（第四步）—— 加上类型擦除三件套
 *
 * 在第三步「带类型的信封」之上，补齐类型擦除：对象安全的 `AnyEnvelope`、
 * 装箱后的 `SealedEnvelope`（`seal` 封箱、`clone_box` 擦除后克隆），
 * 以及零 unsafe 的安全 `downcast`。占位信封 `DummyEnvelope` 留到第五步。
 */
