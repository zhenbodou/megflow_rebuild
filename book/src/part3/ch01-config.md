...
本章只负责最左边那一跳：**文本 → `Config`**。它是纯粹的**解析层**（原版把这一层叫「presentation」）——只管把字段读进结构，**不判断**「`add:a` 里的 `add` 到底存不存在」这类跨引用问题。那类校验集中在 Ch3.2 的 `build()`（这正是本书「**把校验前移到 build()**」这条优化的分工：原版有些校验拖到运行时才发现，我们在建图那一刻就查清）。
...
道理想一下就通：`flatten` 的职责是「兜住其余所有键」，`deny_unknown_fields` 的职责是「拒绝任何未知键」——一个要收、一个要拒，语义直接打架。serde 干脆禁止两者同时出现。而 `NodeConfig` 的本意就是**要收**未知键（那正是 `args`），所以它天然不能要 deny。反过来，`PortConfig` 没有兜底字段，未知键一定是笔误，加上 deny 正好挡住。
...
为什么这么切？因为跨引用校验需要**全局视野**（要同时看到所有节点、所有端口才能判断一个引用对不对），而它天然属于「装配」这道工序。把它放在 `build()` 里，我们就有了一个**单一的、集中的校验点**——一次性把配置错误全查出来、给出好错误，而不是零散地在运行时东一处西一处地 panic。这就是「校验前移到 build()」相对原版的价值。本章的解析层只做它该做的：把文本变成结构，把结构里能免费查的（拼写、必填、格式）用 serde 属性查掉。
...
- **分工**：解析层只「发现」（`main_graph` 返回 `Option`），跨引用校验集中到 Ch3.2 的 `build()`——一个集中的校验点，正是相对原版「校验前移」的价值。
