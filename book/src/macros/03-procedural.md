...
前两课用匹配规则生成代码。本课开始使用 Rust 函数分析和生成代码，这就是过程宏。先实现能运行的 TypeName 派生宏，后面再用它理解 MegFlow 的 Node/Actor 生成器；本课不要求先实现节点。

<!-- toc -->

## 1. 过程宏是什么

先接上第 1、2 课：`macro_rules!`（声明宏）通过匹配语法生成代码。**过程宏**则是**真正的 Rust 程序**：它接收一段代码的 **token 流**作为输入，返回一段 token 流作为输出，中间你可以用任意 Rust 逻辑去分析、变换、生成。它在编译期、在你的代码被真正编译**之前**运行。
...
[课程首页](00-roadmap.md) · [上一课](02-declarative.md) · [下一课](04-tokens.md)
