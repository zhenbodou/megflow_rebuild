...
- **actor（参与者）模型**：图里每个加工环节都是一个处理单元，叫 actor。它拥有自己的状态，主要通过收发消息协作。但 MegFlow 的节点也能通过 `Arc` 和资源集合共享内存；共享可变数据仍需要同步。一个节点变慢会通过通道背压影响上游，任务 panic 的影响则取决于运行时的错误处理，并没有自动的故障隔离保证。
...
连接两个端口的**异步队列**：上游节点往里 `send`，下游节点从里 `recv`。`cap > 0` 表示有界队列；本项目中 `cap = 0` 表示无界队列，不是零容量握手通道。通道也可以被**关闭**。
...
**为什么需要**：把「装配线路」从代码里抽出来变成**配置**。使用已经注册的节点类型调整连线或参数，只需改 TOML；新增 Rust 节点类型或修改其实现仍需要编译。

### 运行时 Runtime

真正让图「跑起来」的调度器。当前静态图把每个节点启动为 Tokio 异步任务。任务进入 `exec` 后，由节点自己决定接收哪些输入；`recv().await` 尚未就绪时让出执行机会，数据到达或关闭事件会唤醒等待任务。运行时并不事先检查「所有输入是否到齐」。
...
3. 节点任务进入 `exec`，并发等待 `a`、`b` 两次接收；两者分别取出 `1` 和 `2` 后继续运算。若消息尚未到达，任务会在这里等待。
...
下面是**原版 API 的阅读示例**，不是本章要写入工程的文件。原版的 `#[amain]`、生命周期方法和图端口调用方式与重构版有差异；不要把这两段拼进 Ch0.2 的空骨架。Ch3.4 会给出重构版的可运行工程与命令。此处先识别数据如何流动，具体语法随后逐步实现。
...
本章不修改工程。阅读后先试着回答：只有 `a` 收到消息时，谁在等待？`b` 也收到后，谁计算结果？这条链的答案是节点中的并发接收、节点的运算和通道发送；框架的完整目标还包括后续的配置、生命周期、消息业务模型和插件。

## 本书路线图

知道了要造什么，再看这本书会怎么一步步把你带到那里。全书分 **6 个 Part**，顺着引擎真实的依赖关系「从地基往上」建：

- **Part 0 · 全景与环境**（你在这里）：建立概念、搭好开发环境，阅读固定版本的原版源码并建立行为验收标准。
...
- **Part 3 · 图与运行时**：TOML 配置解析、Graph Builder 装配节点与 channel、Tokio 调度与停机。**里程碑 Ch3.4**：端到端跑通重构版 BinaryOp `1 + 2 == 3`，完成第一条基础执行链。
