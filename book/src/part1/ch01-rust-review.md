...
这一章先建立后面动手所需的基本词汇，再用一个完整的小程序验证它们。不要先去读最终框架代码；遇到新概念时，先完成当前实验。读完你要能回答四个问题：
...
后面的程序每一行都有具体工作。先记住这些写法，随后在完整实验中逐个找到它们：

| 写法 | 本章中的含义 |
|---|---|
| `let name = value;` | 创建一个绑定，让 name 指向这个值；默认不能给该绑定重新赋值 |
| `let mut name = value;` | 允许修改这个绑定所持有的值；mut 不代表多个地方能同时修改 |
| `fn f(x: u32) -> u32` | 定义函数，参数 x 是无符号 32 位整数，返回值也是 u32 |
| `struct Frame { data: Vec<u8> }` | 定义一个类型，里面有名为 data 的字段，保存可增长的字节数组 |
| `impl Frame { ... }` | 给这个类型编写方法；不是创建一个 Frame 值 |
| `Self` / `self` | 前者指当前实现的类型，后者指方法正在操作的具体对象 |
| `Vec<u8>` / `Option<T>` | 尖括号指定内部元素的类型；Option 有 Some(value) 和 None 两种状态 |
| `use std::sync::Arc;` | 引入标准库中的名字，之后可以简写 Arc；不创建对象，也不下载 crate |
| `assert_eq!(a, b);` | 测试中检查两者相等，不等时让测试失败；感叹号说明这是宏调用 |

函数末尾不带分号的表达式可以作为返回值；带分号通常表示完成语句而不将该值作为返回结果。不要死记“所有行都有分号”，观察下面 new、unpack 等方法的末尾表达式会更清楚。

本章完整程序在第 6 节，只用标准库。前面的短片段分别讲一个概念；只有标为完整文件的内容需要保存到磁盘。
