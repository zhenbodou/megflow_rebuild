...
- **动态子图（dynamic subgraph）没做**——运行期按输入流的条数**动态生成** N 份子图实例（原版用于「每来一路视频流就起一套处理管线」）。这真的需要原版那套嵌套运行时 + 逐消息寻址：实例个数在**运行期**才知道，没法在装配前的静态 `flatten` 里摊开。本章只做**静态**子图（配置里写死几份就是几份）。这是 §3 那个架构抉择必然的代价，也是它诚实的边界。
...
- **诚实的边界**：动态子图（运行期按流数量生成实例）需要另一套架构，划在教学子集外。
