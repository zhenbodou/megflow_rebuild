...
> 独立生命周期与动态实例仍需逐项实现和验证（运行期动态子图实例的**机制**已在
> [Ch4.9](ch09-dynports.md) 补上——create→publish→fetch→route 环路）。这些内容属于本书必做范围。
...
（代价不是没有：内联展开做不了**动态子图**——运行期按流的条数生成 N 份实例。那确实需要一条**运行期**环路而非静态展开——机制见 [Ch4.9](ch09-dynports.md)（本重写版用 broker 广播端点 + 每实例一张 `MainGraph`，而非原版的嵌套运行时 + 逐消息寻址），亦见 §7「诚实的边界」。本章教学子集只做**静态**子图，这已经覆盖了「一个模型喂 N 条同构支路」这个最常见的真实场景。）
...
- **动态子图（dynamic subgraph）没做**——运行期按输入流的条数**动态生成** N 份子图实例（原版用于「每来一路视频流就起一套处理管线」）。实例个数在**运行期**才知道，没法在装配前的静态 `flatten` 里摊开。本章只做**静态**子图（配置里写死几份就是几份）；**运行期**按 key 现装实例的机制（create→publish→fetch→route 环路）已在 [Ch4.9](ch09-dynports.md) 补上（用 broker 广播端点 + 每实例一张 `MainGraph`，而非原版的嵌套运行时），TOML / 派生宏的自动接线糖已在 [Ch4.9a](ch09a-derive-dyn-ports.md) / [Ch4.9b](ch09b-config-auto-wiring.md) / [Ch4.9c](ch09c-dyn-demux-in-graph.md) 补齐。
...
- **诚实的边界**：动态子图（运行期按流数量生成实例）需要一条**运行期**环路而非静态展开——机制已在 [Ch4.9](ch09-dynports.md) 补上（broker 广播端点 + 每实例一张 `MainGraph`），派生 / config 自动接线糖已在 [Ch4.9a](ch09a-derive-dyn-ports.md) / [Ch4.9b](ch09b-config-auto-wiring.md) / [Ch4.9c](ch09c-dyn-demux-in-graph.md) 补齐。

**本章（Ch4.4）小结**：从内部连接、数组端口与广播汇聚（Ch4.1/4.2），到资源与上下文（Ch4.3），再到子图复用（Ch4.4），引擎已具备真实算法仓拓扑的骨架。Part 4 后续继续加深：Ch4.5 Reorder（恢复连续序列）、Ch4.6 算法消息、Ch4.7 弧 Demux（按地址分流）、Ch4.8 Broker（给动态子图广播通知）、Ch4.9 弧 DynPorts（运行期动态子图实例，含派生糖 / config 自动接线 / 注册版 `DynDemux`）。走完 Part 4 才进 **Part 5 · 兼容 · 优化 · 收尾**：Ch5.1 把 API 对齐真实 MegFlow、划清 pplcore/mpp 闭源边界；Ch5.2 逐条对比原版，盘点这一版**为什么 bug 更少、哪些地方更优**；Ch5.3 全景回顾整本书，并为想继续深入（图优化器、debugger、C/Python FFI）的读者指路。
