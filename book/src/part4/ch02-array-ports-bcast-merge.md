...
- **`select_ok` 的公平性没有保证**——它按 future 在 Vec 里的顺序 poll，若第 0 路一直有数据，可能**饿死**后面几路（本章测试因数据有限没暴露，但长流下要留意）。原版的 merge 有更讲究的轮转策略，留待 Part 5 的「优化对照」再谈。
- **`NoopProducer`（零输入源节点）仍然没做**——Ch4.1 推迟的这一项，本章也没碰。数组端口解决的是「一名多端」，与「源节点如何自终止」正交，仍留给真正的 source 节点。

不过，Ch4.1 里那条「对外输入扇出到多端口 → `Unsupported`」的限制，本章给了**正解**：不是去解除它（让一个对外输入喂多个端口，那会破坏 mpsc 单消费者），而是**放一个 `Bcast` 进去**——对外输入喂 `Bcast:inp`，`Bcast` 再扇出。限制还在，但绕过它的正确姿势已经有了。
